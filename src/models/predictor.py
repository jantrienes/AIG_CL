...
class Translator(object):
    ...
    def _fast_translate_batch(self,
        ...
        edges = batch.edges
        entities_findings = batch.entities_findings

        src_features,h_b = self.model.encoder(src, segs, mask_src, node_id=node_id, edges=edges,entities_findings=entities_findings)
        ...
        if h_b == None:
            # import pdb
            # pdb.set_trace()
            h_b = torch.zeros(size=src_features.size()).cuda()
        tile_h_b = tile(h_b, beam_size, dim=0)
        tile_node_id = tile(node_id, beam_size, dim=0)
        tile_entities_findings = tile(entities_findings, beam_size, dim=0)
        tile_edges = tile(edges, beam_size, dim=0)
        ...
        for step in range(max_length):
            ...
            dec_out, dec_states,_, _ = self.model.decoder(decoder_input, src_features, dec_states,
                                                     step=step,h_b = tile_h_b,node_id=tile_node_id, edges=tile_edges,entities_findings=tile_entities_findings)
            ...
            tile_entities_findings = tile_entities_findings.index_select(0, select_indices)


            tile_edges = tile_edges.index_select(0, select_indices)
            tile_h_b = tile_h_b.index_select(0, select_indices)
